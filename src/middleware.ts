...
//
// NOTE: this file MUST be named exactly `middleware.ts` and export a
// function named exactly `middleware` — Next.js's build only picks up
// route middleware from that specific file/export name. A differently
// named file (e.g. proxy.ts / a `proxy` export) is silently never run,
// which is what happened before this fix — the gate below looked correct
// but never executed.
